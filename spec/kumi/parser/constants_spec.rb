...
RSpec.describe 'Float::INFINITY Support' do
  ...
  describe 'parsing' do
    it 'parses Float::INFINITY as a literal' do
      ...
      expect(ast.attributes.first.expression.value).to eq(Float::INFINITY)
      expect(ast.attributes.first.expression.value.infinite?).to eq(1)
    end
    ...
    it 'parses Float::INFINITY in arrays' do
      ...
      values = ast.attributes.first.expression.elements.map(&:value)
  ...
  describe 'integration with tax calculation' do
    it 'supports Float::INFINITY in tax bracket arrays like Ruby DSL' do
      ...
      brackets = ast.attributes.first.expression.elements.map(&:value)
